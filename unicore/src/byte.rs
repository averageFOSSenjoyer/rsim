...
impl<const T: usize, const R: usize> ByteOrd<Bytes<R>> for Bytes<T> {
    fn byte_cmp(self, other: Bytes<R>) -> Ordering {
        ...
        for i in (0..T).rev() {
            if self[i].unwrap() > other[i].unwrap() {
                return Ordering::Greater;
            } else if self[i].unwrap() < other[i].unwrap() {
                return Ordering::Less;
            }
        }
...
// impl<const T: usize> Eq for Bytes<T> {}
//
// impl<const T: usize> PartialEq<Self> for Bytes<T> {
...
// impl<const T: usize> PartialOrd<Self> for Bytes<T> {
//     fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
//         Some(self.byte_cmp(*other))
//     }
// }

// impl<const T: usize> Ord for Bytes<T> {
...
impl<const T: usize, const R: usize> SignedOrd<Bytes<R>> for Bytes<T> {
    fn signed_cmp(self, rhs: Bytes<R>) -> Ordering {
        ...
        let mut self_no_msb = self.clone();
        self_no_msb[T - 1] = Some(self_no_msb[T - 1].unwrap() & 0x7F);
        let mut other_no_msb = rhs.clone();
